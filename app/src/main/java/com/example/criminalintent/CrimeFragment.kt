...
import java.io.File
import java.util.*
import android.provider.ContactsContract // Для работы с контактами
import android.provider.MediaStore // Для работы с камерой и фото
...
class CrimeFragment : Fragment(), DatePickerFragment.Callbacks {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Устанавливаем слушателя для получения результата из DatePickerFragment
        parentFragmentManager.setFragmentResultListener(
            DatePickerFragment.RESULT_KEY, viewLifecycleOwner
        ) { _, bundle ->
            val date = bundle.getSerializable(DatePickerFragment.RESULT_DATE) as Date
            onDateSelected(date)
        }
    }
    ...
    override fun onStart() {
        ...
        dateButton.setOnClickListener {
            // Используем parentFragmentManager для отображения DatePickerFragment
    ...
    private fun updateUI() {
        ...
        suspectButton.text = if (crime.suspect.isNotEmpty()) crime.suspect else ""
        updatePhotoView()
    }
